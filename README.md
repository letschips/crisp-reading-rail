...
Open **Settings → Crisp Reading Rail → Orb style** to choose Default, Random per day, 42 material/character styles, or **Follow Crisp File Explorer**. Follow mode observes only the companion orb's live `data-orb-style` value in the same Obsidian window; if it is unavailable, the rail uses Default.
